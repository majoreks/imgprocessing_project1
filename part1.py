...
for file in os.listdir(plantsPath):
    ...
    #cv.imwrite("green/"+"green_"+file.split("_", 1)[1], green) # to save green only image
    ...
    #cv.imwrite("before_opening/"+"beforeOpening_"+file.split("_", 1)[1], thresh) # to save green only image
    opening = cv.morphologyEx(thresh, cv.MORPH_OPEN, kernel)
    #cv.imwrite("before_closing/"+"beforeClosing_"+file.split("_", 1)[1], opening) # to save green only image
...
# f = open("results/part1_results.txt", "w")
...
#     f.write("Plant " + str(i) + " dice: " + str(diceCoeff[i]) + " jaccard: "
#         + str(jaccardIndex[i]) + "\n")
